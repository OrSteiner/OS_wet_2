...
struct runqueue {
	...
	task_t *curr, *idle;
	prio_array_t *active, *expired,*SHORT arrays[3];
...
repeat_lock_task:
	...
	if (!p->array) {
		...
		{
			if(p->policy!=SCHED_SHORT && p->prio < MAX_RT_PRIO )
				resched_task(rq->curr);
			if(p->policy==SCHED_SHORT && p->short_prio < rq->curr->short_prio)
				resched_task(rq->curr);
		}
...
void scheduler_tick(int user_tick, int system)
{
	...
	if (!--p->time_slice) {
		...
		else if (!TASK_INTERACTIVE(p) || EXPIRED_STARVING(rq)) {
...
pick_next_task:
	...
	{
		array=rq->SHORT;
		if(!array->nr_active || sched_find_first_bit(rq->active->bitmap) < MAX_RT_PRIO)
			array= rq->active;
	}
	else
		array = rq->active;
...
void set_user_nice(task_t *p, long nice)
{
	...
	prio_array_t *array;
	runqueue_t *rq;
	if(p->policy==SCHED_SHORT || current->policy==SCHED_SHORT){
		errno=EPERM;
		return;
	}
...
static int setscheduler(pid_t pid, int policy, struct sched_param *param)
{
	...
	{
		if(lp.requested_time < 1 || lp.requested_time > 3000 || lp.sched_short_prio < 0 || lp.sched_short_prio > 139)
			goto out_unlock;
		retval = -EPERM;
		if ((current->euid != p->euid) && (current->euid != p->uid) &&
	    !capable(CAP_SYS_NICE))
		goto out_unlock; 
		// need to check if we need this condition
		array = p->array;
		if (array)
...
asmlinkage long sys_sched_getparam(pid_t pid, struct sched_param *param)
{
	...
	lp.requested_time = p->time_slice;
